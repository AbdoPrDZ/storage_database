...
abstract class LaravelEchoMigration extends StorageCollection {
  ...
  setup() {
    if (eventsNames.containsKey(EventsType.create)) {
      channel.listen(eventsNames[EventsType.create]!, onCreate);
    }
    if (eventsNames.containsKey(EventsType.update)) {
      channel.listen(eventsNames[EventsType.update]!, onUpdate);
    }
    if (eventsNames.containsKey(EventsType.delete)) {
      channel.listen(eventsNames[EventsType.delete]!, onDelete);
    }
  }
  ...
  onDelete(Map data) async {
    ...
    try {
      ...
      log(
        '[StorageDatabase.LaravelEcho.MigrationError$migrationName]: Cant delete item $id.\n error: "$e"',
      );
    }
  }
}
